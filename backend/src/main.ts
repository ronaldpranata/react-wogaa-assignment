...
async function bootstrap(): Promise<void> {
  ...
  if (appConfig.nodeEnv !== 'production') {
    const swaggerConfig = new DocumentBuilder()
      .setTitle('WOGAA Items API')
      .setDescription('NestJS REST API — Items CRUD with SQLite')
      .setVersion('1.0')
      .addTag('Items', 'CRUD operations for the Items resource')
      .build();
  ...
  logger.log(`🛡  CORS allowed origins: ${appConfig.allowedOrigins.join(', ')}`);
}
